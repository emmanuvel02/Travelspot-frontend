...
import { FindBookings,CanelBooking } from '../../../configure/user';
...
export default function BookingView() {
  ...
  const findbooking=async()=>{
    try {
        const response=await FindBookings()
  ...
  const handleCancelBooking =async(id) => {
   try {
    const response=await CanelBooking(id)
        if(response.data.success){
            findbooking()
        ...
        <table className="min-w-full bg-white rounded-lg overflow-hidden">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="py-3 px-4 text-center font-semibold text-gray-700">CancelBooking</th>
